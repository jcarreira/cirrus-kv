...
namespace cirrus_terasort {
	...
	read_lambda::read_lambda(INT_TYPE proc_num) {
		INT_TYPE per_process = config_instance::num_records / config_instance::num_input_files;
	...
	INT_TYPE read_lambda::read_input_chunk(std::shared_ptr<cirrus::ostore::FullBladeObjectStoreTempl<std::string>> ostore) {
