...
void train(
    ...
    double w[d + 1]; // this is our model

    for (int i = 0; i < d + 1; ++i)
        w[i] = 1.0 * rand() / RAND_MAX - 0.5;
    ...
    while (1) {
        ...
        for (int m = 0; m < minibatch; ++m) {
            ...
            samples_store.get(sample_idx, (T*)sample);
            sample[d] = 1;

            labels_store.get(nsamples + sample_idx, (C*)&label);
...
auto main(int argc, char** argv) -> int {
    ...
    cirrus::ostore::FullBladeObjectStoreTempl<sample_struct> samples_store(IP, PORT);
    cirrus::ostore::FullBladeObjectStoreTempl<double> labels_store(IP, PORT);
    load_input_data(samples_store, labels_store, labels, samples);

    train(samples_store, labels_store,
            labels, samples, samples.size(), samples[0].size());
