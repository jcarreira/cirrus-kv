...
RedisIterator::RedisIterator(uint64_t left_id, uint64_t right_id, const std::string& IP, int port) : left_id(left_id), right_id(right_id), IP(IP), port(port) {
   ...
   while (1) {
      ...
      if (ring.size() < read_ahead) {
        ...
        for (uint64_t i = 0; i < to_read; ++i) {
           //std::cout << "Get batch with id: " << last
           //          << std::endl;
