...
void test_1_client() {
    ...
    client1.write_sync(0, to_send.c_str(), std::strlen(to_send.c_str()));

    client1.read_sync(0, data, std::strlen(to_send.c_str()));

    if (strncmp(data, to_send.c_str(), std::strlen(to_send.c_str())))
...
void test_2_clients() {
    ...
    client2.write_sync(0, message.c_str(), std::strlen(message.c_str()));
    ...
    client2.read_sync(0, data, std::strlen(message.c_str()));
    cirrus::LOG<cirrus::INFO>("Received data 2: ", data);

    // Check that client2 receives "data2"
    if (strncmp(data, message.c_str(), std::strlen(message.c_str())))
        throw std::runtime_error("Error in test");
}
