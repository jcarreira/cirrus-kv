...
#include "common/schemas/CoordinatorMessage_generated.h"
...
/**
  ...
  * Given a port and queue length, sets the values
  * of the variables.
  ...
  */
Coordinator::Coordinator(int port, uint64_t max_fds_) :
    port_(port), max_fds(max_fds_ + 1) {
        if (max_fds_ + 1 == 0) {
...
void Coordinator::init() {
    ...
    if (server_sock_ < 0) {
        throw cirrus::ConnectionException("Coordinator error creating socket");
    }
    ...
    LOG<INFO>("Created socket in Coordinator");
 ...
 */
bool Coordinator::testRemove(struct pollfd x) {
...
/**
  * Coordinator processing loop. When called, server loops infinitely, accepting
    ...
    while (1) {
        ...
        if (curr_index == max_fds) {
            ...
            std::remove_if(fds.begin(), fds.end(),
                std::bind(&Coordinator::testRemove, this,
                    std::placeholders::_1));
        }
    }
}
 ...
 */
ssize_t Coordinator::send_all(int sock, const void* data, size_t len,
    int /* flags */) {
    ...
    while (to_send != total_sent) {
        ...
        if (sent == -1) {
            LOG<ERROR>("Coordinator error sending data to client, "
 ...
 * @return the number of bytes sent.
 */
ssize_t Coordinator::read_all(int sock, void* data, size_t len) {
...
/**
  ...
  */
bool Coordinator::read_from_client(
    ...
    while (bytes_read < static_cast<int>(sizeof(uint32_t))) {
        ...
        if (retval < 0) {
            throw cirrus::Exception("Coordinator issue in reading "
 ...
 */
bool Coordinator::process(int sock) {
...
#ifdef PERF_LOG
    LOG<PERF>("Coordinator::process resize time (us): ",
    ...
    LOG<PERF>("Coordinator::process receive time (us): ",
            receive_time.getUsElapsed(),
            " bw (MB/s): ", recv_mbps);
#endif
    LOG<INFO>("Coordinator received full message from client");

    // Extract the message from the buffer
    auto msg = message::CoordinatorMessage::GetCoordinatorMessage(buffer.data());
    ...
    LOG<INFO>("Coordinator checking type of message");
    // Check message type
    //bool success = true;
    switch (msg->message_type()) {
        case message::CoordinatorMessage::Message_RequestMemory:
            {
                uint64_t amount;
#ifdef PERF_LOG
                TimerFunction write_time;
#endif
                LOG<INFO>("Coordinator processing RequestMemory request.");

                // first see if the object exists on the server.
                // If so, overwrite it and account for the size change.
                amount = msg->message_as_RequestMemory()->amount();

                LOG<INFO>("Client request more memory: ", amount);
    ...
    if (send_all(sock, &network_order_size, sizeof(uint32_t), 0) == -1) {
        LOG<ERROR>("Coordinator error sending message back to client. "
            "Possible client died");
        return false;
    }

    LOG<INFO>("Coordinator sent size.");
    ...
    LOG<PERF>("Coordinator::process reply time (us): ",
            reply_time.getUsElapsed(),
            " bw (MB/s): ", reply_mbps);
#endif

    LOG<INFO>("Coordinator sent ack of size: ", message_size);
    LOG<INFO>("Coordinator done processing message from client");
    return true;
}
